...
| 加载镜像或动态库（Image/Library Loaded） | `process/image_load` | Target 的系统目录 LoadLibraryW、微软 DLL 应用目录 LoadLibraryW/LoadLibraryExW、无签名第三方原生 DLL LoadLibraryW + 导出调用，以及 `dotnet.exe` Helper 的 AssemblyLoadContext 托管程序集加载五个子项；各自的目标角色/PID、发生时间、方法、源路径、实际模块路径、文件名、基址、大小、hash、加载前后枚举；原生导出解析/调用结果；临时 DLL 清理结果 |
