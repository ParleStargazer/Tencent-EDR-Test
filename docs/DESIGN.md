...
6. 以本地文件路径、程序路径/命令行、PID 等强中弱锚点为唯一评分依据，再用时间距离排序；`event.type`/`event.action` 只作展示提示，既不加分也不负责过滤候选。一个能力含多个行为子项时，可由每个 `cloud_expectation.correlation` 覆盖关联锚点和本地发生时间；
7. 优先选择达到本地锚点阈值的唯一最佳候选执行字段、数量和时间断言；若没有强匹配，仍保留时间相近、仅命中部分文件/程序条件的低置信度 JSON 块，并继续比较所有可读取字段。某个 EDR 字段缺失只影响该字段，不得中止后续断言；
8. 检查云端导出范围是否足以支持“未发现”结论；
9. 输出逐能力结果、逐字段结果、当前能力的本地导出 JSON 块、排序后的完整候选日志、逐候选 BASELINE 匹配位置、输入 hash 和中文总体结论。
...
前端为每项能力提供只读 JSON 对照悬浮窗：左侧展示按 `case_run_id` 截取的本地运行导出块，右侧展示用户选择的 EDR 原始候选块。通过的本地要求、关联锚点和云端断言使用后端提供的 JSON Pointer 精确高亮；候选切换会同步重算显示该候选自己的命中字段，但不会人工篡改自动验证结论。
