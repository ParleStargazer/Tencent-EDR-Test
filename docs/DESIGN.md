...
6. 按强、中、弱锚点评分；一个能力含多个行为子项时，可由每个 `cloud_expectation.correlation` 覆盖关联锚点和本地发生时间；同分时按 EDR 事件与对应本地行为时间的距离由近到远排序；
7. 对唯一最佳候选执行字段、数量和时间断言，同时对每条候选独立计算关联锚点与字段断言，保留规范化字段、EDR 原始完整日志及原始字段 JSON Pointer；
