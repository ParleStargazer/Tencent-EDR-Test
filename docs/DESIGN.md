...
同一能力内的执行子测试也必须串行触发，默认间隔 1000 ms，可配置为 0–10000 ms。该间隔从前一子测试完成行为采集、独立复核并保存本地证据后开始，到下一子测试放行前结束。等待期间允许观察进程驻留，但不允许两个测试方法同时执行行为；Runner 会把实际配置写入 `execution.inter_subtest_delay_ms`，Controller 以 `SUBTEST_WAITING` 日志报告方法切换。能力清单的 `execute_seconds` 必须包含最大子测试间隔预算；当前最多五方法的 DLL 加载能力已按四次最大间隔扩展执行超时。
...
当一项能力包含不同文件类型、加载方式或其他可替代测试路径时，BASELINE 可为每个 `cloud_expectation` 声明稳定的 `method.id` 与中英可读标题，并通过 `method_selection.strategy: best` 启用最佳方法结论。只有一条可验证路径的能力也可使用相同结构，统一输出方法结果。比较器仍先把本地条件作为整项能力的绝对前置基准；本地失败不能被任何云端方法覆盖。云端方法分别关联、评分和验证，默认按 `PASS > PARTIAL > INCONCLUSIVE > FAIL` 选择状态最好的方法，状态相同时依次比较候选关联分数、时间差绝对值和 BASELINE 顺序。结果同时保存被选方法、选择提示及全部方法的逐字段证据，前端默认展开被选方法，其余方法可独立展开排查。
...
同一 Canonical 字段在厂商不同版本中存在多个原始字段别名时，字段规则可以使用按优先级声明的 `sources`；映射器选择第一项非空字段，并把实际命中的原始字段保存到比较报告的 JSON Pointer。`sources` 不得与 `source` 或 `constant` 同时声明。
...
Group Policy Modification 使用独立能力包写入 `HKLM\SOFTWARE\Policies\EdrTest\Runs\<nonce>\ValidationMarker`，只修改无实际策略语义的本轮字符串值。Controller 预置和独立复核，Actor 调用 `RegSetValueExW`，随后按精确 nonce 子键清理。真实腾讯样本表明组策略仍由 `RegEvents / RegSetValue` 承载，直接语义字段为 `Child.RegGroupName=组策略`。Named Pipe Activity 使用 Actor、Helper 双进程和 Win32 `CreateNamedPipeW`/`CreateFileW`，两端必须完成双向 nonce 握手；创建项由 Actor 充当服务端，连接项由 Actor 充当客户端。两项均使用本轮 `\\.\pipe\EdrTest_<nonce>_<operation>`，管道随进程退出释放。完整约束见 `docs/GROUP-POLICY-ACTIVITY-SAMPLES.md` 与 `docs/NAMED-PIPE-ACTIVITY-SAMPLES.md`。

Scheduled Task Activity 三项能力操作本轮唯一的 `\EdrTest_<nonce>_<operation>_<method>` 当前用户任务，创建、修改、删除都运行 Task Scheduler 2.0 COM、原生 `schtasks_cli` 与 `security_audit_*` 三个独立方法。COM 创建/更新用于验证 `ServiceEvents / InjectHook / RpcSchedTaskCreate` 注册调用链，该动作规范化为 `register`，不能单独区分创建和修改；原 CLI 分别执行 `/Create`、无凭据交互的 `/Change /ENABLE`、`/Delete`，安全事件只作诊断，不影响其本地任务状态结论；新增安全审计方法精确保存并临时启用“其他对象访问事件”成功审核，通过 `/Create /XML /F` 或 `/Delete` 激发并严格读取 4698、4702、4699，然后恢复原策略。创建任务虽启用但只有一年后的单次触发器；修改预置任务无触发器，即使启用也不会执行。所有动作无害且立即精确清理，测试过程从不启动任务，能力整体属于 L2。BASSLINE 以本地任务状态为绝对基准，三个方法分别比较并采用最佳结论；安全审计方法以本机 Security 事件时间作为强关联证据。`SchedTaskDelete` 等待测试机实测。完整约束见 `docs/SCHEDULED-TASK-ACTIVITY-SAMPLES.md`。
