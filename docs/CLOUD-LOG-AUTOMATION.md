...
自动化采用逐步骤状态等待，而不是假定页面会在固定短时间内完成加载：普通控件最多等待 60 秒；登录提交后的域选择或 EDR 事件视图最多等待 180 秒；提交登录后先等待 15 秒，再开始定位域选择器或 EDR 事件视图；每个主要步骤完成后默认预留 1600 毫秒页面稳定时间；提交时间筛选后等待 6 秒让检索结果加载。页面导航与文件下载仍分别允许最多 5 分钟，Node 自动化子进程的总执行窗口为 10 分钟。
