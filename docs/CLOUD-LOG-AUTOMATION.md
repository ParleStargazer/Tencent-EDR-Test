...
2. 登录密码；
3. 设备名称，默认使用本机计算机名；
4. 可选日志起始时间，留空时使用 Runner 记录的轮次实际开始时间前 10 秒；
5. 测试结束后等待时间，默认 30 秒，范围 0–3600 秒；
6. 可选“调试模式（浏览器界面可见）”。启用后使用可见 Edge，页面实时展示详细自动化事件，并在本轮目录保存完整的脱敏 JSONL 调试日志。

自动查询范围的结束时间固定使用 Runner 封存本地轮次的实际结束时间后 10 秒。查询窗口不以 API 接收请求或浏览器自动化启动的时间近似计算；用户手动填写起始时间时只覆盖查询下界，不改变结束时间。

“高级等待选项”默认收起，可按测试网络和腾讯云页面加载情况调整六类参数。前端、本地 API 和 Node 自动化入口使用同一组范围约束：

| 参数 | 默认值 | 允许范围 | 作用 |
|---|---:|---:|---|
| 普通控件超时 | 60000 ms | 1000–300000 ms | 定位、点击或填写普通控件的最长等待 |
| 登录后状态超时 | 180000 ms | 1000–300000 ms | 等待域选择器或 EDR 事件页出现 |
| 步骤稳定等待 | 1600 ms | 0–5000 ms | 每个主要步骤完成后的固定等待 |
| 域定位前等待 | 15000 ms | 0–30000 ms | 登录提交后、定位域选择器前的固定等待 |
| 筛选动作前后等待 | 150 ms | 0–2000 ms | 添加主机名和时间条件时，每个 UI 动作前后分别等待 |
| 检索结果等待 | 6000 ms | 0–30000 ms | 时间范围检索提交后、开始导出前的固定等待 |
...
5. 添加“采集时间”，选择“在范围内”，依次填写开始、结束两个时间框，在第二个时间框按 Enter，再点击筛选弹层中的“检索”；
6. 检索后打开导出对话框，选择全部字段和 JSON 格式；
7. 等待浏览器下载事件，将文件保存到当前 run。

自动化采用逐步骤状态等待，而不是假定页面会在固定短时间内完成加载。上述数值均为高级选项的默认值；页面导航与文件下载仍分别允许最多 5 分钟，Node 自动化子进程的总执行窗口为 10 分钟。

域选择步骤默认通过 `domain_selection_wait` 记录 15 秒渲染等待（实际数值取“域定位前等待”高级选项），等待结束后才开始定位 EDR 全部事件视图或域下拉控件。域下拉首先通过“请选择域”占位文字限定正确的 `.app-ioa-dropdown` 容器，再点击 `.app-ioa-dropdown__header.app-ioa-dropdown-btn`，不把内部 SVG 图标当作主点击目标；`#ioa-v1 #chevron-down`、兼容文本定位和已知绝对 XPath 仅作为回退。只有等待后已经看到事件视图时，才判定当前账号不需要选择域；定位到 header 后会滚动到可视区域，并使用 Playwright `trial` 点击预检确认没有动画或遮挡，再选择默认域。等待或预检失败会停在 `select_domain` 并返回稳定错误码，不再串联执行后续错误步骤。调试日志中的 `domain_chooser_actionable` 表示 header 已通过点击预检。
