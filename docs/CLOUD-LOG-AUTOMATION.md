...
2. 登录密码；
3. 设备名称，默认使用本机计算机名；
4. 可选日志起始时间，留空时使用 Runner 记录的轮次实际开始时间前 30 秒；
5. 测试结束后等待时间，默认 30 秒，范围 0–3600 秒；
6. 可选“调试模式（浏览器界面可见）”。启用后使用可见 Edge，页面实时展示详细自动化事件，并在本轮目录保存完整的脱敏 JSONL 调试日志。

自动查询范围的结束时间固定使用 Runner 封存本地轮次的实际结束时间后 30 秒。查询窗口不再以 API 接收请求或浏览器自动化启动的时间近似计算；用户手动填写起始时间时只覆盖查询下界，不改变结束时间。
...
自动化采用逐步骤状态等待，而不是假定页面会在固定短时间内完成加载：普通控件最多等待 60 秒；登录提交后的域选择或 EDR 事件视图最多等待 180 秒；提交登录后先等待 15 秒，再开始定位域选择器或 EDR 事件视图；添加主机名和事件时间条件时，每次定位并执行 UI 操作前、成功完成后各等待 150 毫秒，其中包括首次“添加筛选条件”和后续“添加条件”；每个主要步骤完成后默认预留 1600 毫秒页面稳定时间；提交时间筛选后等待 6 秒让检索结果加载。页面导航与文件下载仍分别允许最多 5 分钟，Node 自动化子进程的总执行窗口为 10 分钟。

域选择步骤先通过 `domain_selection_wait` 记录 15 秒渲染等待，等待结束后才开始定位 EDR 全部事件视图或域下拉控件。域下拉首先通过“请选择域”占位文字限定正确的 `.app-ioa-dropdown` 容器，再点击 `.app-ioa-dropdown__header.app-ioa-dropdown-btn`，不把内部 SVG 图标当作主点击目标；`#ioa-v1 #chevron-down`、兼容文本定位和已知绝对 XPath 仅作为回退。只有等待后已经看到事件视图时，才判定当前账号不需要选择域；定位到 header 后会滚动到可视区域，并使用 Playwright `trial` 点击预检确认没有动画或遮挡，再选择默认域。等待或预检失败会停在 `select_domain` 并返回稳定错误码，不再串联执行后续错误步骤。调试日志中的 `domain_chooser_actionable` 表示 header 已通过点击预检。
