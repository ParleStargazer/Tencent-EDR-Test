...
static VOID UsbUdeWriteInitializationDiagnostic(
    ...
    _In_ NTSTATUS Status)
{
    ...
    UNICODE_STRING interfaceGuidName;
    UNICODE_STRING stageValue;
    UNICODE_STRING interfaceGuidValue;

    registryStatus = WdfDriverOpenParametersRegistryKey(
        Driver,
        KEY_SET_VALUE,
    ...
    RtlInitUnicodeString(&stageName, L"InitializationStage");
    RtlInitUnicodeString(&statusName, L"InitializationStatus");
    RtlInitUnicodeString(&interfaceGuidName, L"InterfaceGuid");
    RtlInitUnicodeString(&stageValue, Stage);
    RtlInitUnicodeString(&interfaceGuidValue, L"{77DC40F2-80FB-4F86-A6D4-793AB56D2D45}");
    (VOID)WdfRegistryAssignUnicodeString(key, &stageName, &stageValue);
    (VOID)WdfRegistryAssignULong(key, &statusName, (ULONG)Status);
    (VOID)WdfRegistryAssignUnicodeString(key, &interfaceGuidName, &interfaceGuidValue);
    ...
    _In_ PUNICODE_STRING RegistryPath)
{
    ...
    if (NT_SUCCESS(status)) {
        UsbUdeWriteInitializationDiagnostic(driver, L"driver_entry_succeeded", status);
    }
    return status;
}
    ...
    _Inout_ PWDFDEVICE_INIT DeviceInit)
{
    ...
    UsbUdeWriteInitializationDiagnostic(Driver, L"evt_device_add_entered", STATUS_SUCCESS);
    ...
    if (!NT_SUCCESS(status)) {
        UsbUdeWriteInitializationDiagnostic(Driver, L"sddl_assignment_failed", status);
        return status;
    }
    UsbUdeWriteInitializationDiagnostic(Driver, L"sddl_assignment_succeeded", status);
    status = UdecxInitializeWdfDeviceInit(DeviceInit);
    if (!NT_SUCCESS(status)) {
        UsbUdeWriteInitializationDiagnostic(Driver, L"udecx_device_init_failed", status);
        return status;
    }
    ...
    UsbUdeWriteInitializationDiagnostic(Driver, L"wdf_device_create_succeeded", status);
    ...
    UsbUdeWriteInitializationDiagnostic(Driver, L"udecx_emulation_add_succeeded", status);

    status = WdfDeviceCreateDeviceInterface(device, &GUID_DEVINTERFACE_USB_UDE_TEST, NULL);
    if (!NT_SUCCESS(status)) {
        UsbUdeWriteInitializationDiagnostic(Driver, L"device_interface_create_failed", status);
        return status;
    }
    UsbUdeWriteInitializationDiagnostic(Driver, L"device_interface_create_succeeded", status);
    ...
    if (!NT_SUCCESS(status)) {
        UsbUdeWriteInitializationDiagnostic(Driver, L"internal_queue_create_failed", status);
        return status;
    }
    UsbUdeWriteInitializationDiagnostic(Driver, L"internal_queue_create_succeeded", status);

    WDF_IO_QUEUE_CONFIG_INIT_DEFAULT_QUEUE(&queueConfig, WdfIoQueueDispatchSequential);
    queueConfig.EvtIoDeviceControl = UsbUdeEvtIoDeviceControl;
    queueConfig.PowerManaged = WdfFalse;
    status = WdfIoQueueCreate(device, &queueConfig, WDF_NO_OBJECT_ATTRIBUTES, WDF_NO_HANDLE);
    UsbUdeWriteInitializationDiagnostic(
        Driver,
