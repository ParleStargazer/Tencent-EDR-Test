...
$publishRoot = Join-Path $repositoryRoot "artifacts\bits-activity-publish"
...
dotnet publish (Join-Path $repositoryRoot "sample-src\BitsActivity\BitsActivity.Controller\BitsActivity.Controller.csproj") `
    --configuration $Configuration --no-restore --output $controllerPublish
if ($LASTEXITCODE -ne 0) { throw "BitsActivity Controller 发布失败。" }
dotnet publish (Join-Path $repositoryRoot "sample-src\BitsActivity\BitsActivity.Behavior\BitsActivity.Behavior.csproj") `
    --configuration $Configuration --no-restore --output $behaviorPublish
if ($LASTEXITCODE -ne 0) { throw "BitsActivity Behavior 发布失败。" }

$capabilityId = "win.bits.job"
...
Copy-Item (Join-Path $controllerPublish "BitsActivity.Controller.exe") (Join-Path $destination "BitsJob.Controller.exe") -Force
Copy-Item (Join-Path $behaviorPublish "BitsActivity.Behavior.exe") (Join-Path $destination "BitsJob.Actor.exe") -Force
$manifest = Get-Content (Join-Path $repositoryRoot "sample-src\BitsActivity\manifests\$capabilityId\capability.json") -Raw | ConvertFrom-Json -Depth 30
...
Write-Host "BITS Activity 能力包构建完成：$SamplesRoot"
