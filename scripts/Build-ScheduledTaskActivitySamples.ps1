...
$publishRoot = Join-Path $repositoryRoot "artifacts\scheduled-task-activity-publish"
...
if ($LASTEXITCODE -ne 0) { throw "ScheduledTaskActivity Controller 发布失败。" }

dotnet publish (Join-Path $repositoryRoot "sample-src\ScheduledTaskActivity\ScheduledTaskActivity.Behavior\ScheduledTaskActivity.Behavior.csproj") `
    --configuration $Configuration --no-restore --output $behaviorPublish
if ($LASTEXITCODE -ne 0) { throw "ScheduledTaskActivity Behavior 发布失败。" }

$packages = @(
    @{ Id = "win.scheduled_task.create"; Prefix = "ScheduledTaskCreate" },
    @{ Id = "win.scheduled_task.modify"; Prefix = "ScheduledTaskModify" },
    @{ Id = "win.scheduled_task.delete"; Prefix = "ScheduledTaskDelete" }
)
...
foreach ($package in $packages) {
    ...
    Copy-Item (Join-Path $controllerPublish "ScheduledTaskActivity.Controller.exe") `
        (Join-Path $destination "$($package.Prefix).Controller.exe") -Force
    Copy-Item (Join-Path $behaviorPublish "ScheduledTaskActivity.Behavior.exe") `
        (Join-Path $destination "$($package.Prefix).Actor.exe") -Force

    $manifestTemplate = Join-Path $repositoryRoot "sample-src\ScheduledTaskActivity\manifests\$($package.Id)\capability.json"
...
Write-Host "计划任务活动三个能力包构建完成：$SamplesRoot"
