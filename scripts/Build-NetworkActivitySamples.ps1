...
dotnet publish (Join-Path $repositoryRoot "sample-src\NetworkActivity\NetworkActivity.Controller\NetworkActivity.Controller.csproj") `
    --configuration $Configuration --no-restore --output $controllerPublish
if ($LASTEXITCODE -ne 0) { throw "NetworkActivity Controller 发布失败。" }

dotnet publish (Join-Path $repositoryRoot "sample-src\NetworkActivity\NetworkActivity.Behavior\NetworkActivity.Behavior.csproj") `
    --configuration $Configuration --no-restore --output $behaviorPublish
if ($LASTEXITCODE -ne 0) { throw "NetworkActivity Behavior 发布失败。" }

$packages = @(
    @{ Id = "win.network.tcp"; Prefix = "NetworkTcp" },
    @{ Id = "win.network.udp"; Prefix = "NetworkUdp" },
    @{ Id = "win.network.url"; Prefix = "NetworkUrl" },
    @{ Id = "win.network.dns"; Prefix = "NetworkDns" },
    @{ Id = "win.network.file_download"; Prefix = "NetworkFileDownload" }
)
...
foreach ($package in $packages) {
    ...
    Copy-Item (Join-Path $controllerPublish "NetworkActivity.Controller.exe") `
        (Join-Path $destination "$($package.Prefix).Controller.exe") -Force
    Copy-Item (Join-Path $behaviorPublish "NetworkActivity.Behavior.exe") `
        (Join-Path $destination "$($package.Prefix).Actor.exe") -Force
    Copy-Item (Join-Path $behaviorPublish "NetworkActivity.Behavior.exe") `
        (Join-Path $destination "$($package.Prefix).Helper.exe") -Force

    $manifestTemplate = Join-Path $repositoryRoot "sample-src\NetworkActivity\manifests\$($package.Id)\capability.json"
...
Write-Host "Network Activity 五个能力包构建完成：$SamplesRoot"
