...
$publishRoot = Join-Path $repositoryRoot "artifacts\service-activity-publish"
...
if ($LASTEXITCODE -ne 0) { throw "ServiceActivity Controller 发布失败。" }

dotnet publish (Join-Path $repositoryRoot "sample-src\ServiceActivity\ServiceActivity.Behavior\ServiceActivity.Behavior.csproj") `
    --configuration $Configuration --no-restore --output $behaviorPublish
if ($LASTEXITCODE -ne 0) { throw "ServiceActivity Behavior 发布失败。" }

$packages = @(
    @{ Id = "win.service.create"; Prefix = "ServiceCreate" },
    @{ Id = "win.service.modify"; Prefix = "ServiceModify" },
    @{ Id = "win.service.delete"; Prefix = "ServiceDelete" }
)
...
foreach ($package in $packages) {
    ...
    Copy-Item (Join-Path $controllerPublish "ServiceActivity.Controller.exe") `
        (Join-Path $destination "$($package.Prefix).Controller.exe") -Force
    Copy-Item (Join-Path $behaviorPublish "ServiceActivity.Behavior.exe") `
        (Join-Path $destination "$($package.Prefix).Actor.exe") -Force

    $manifestTemplate = Join-Path $repositoryRoot "sample-src\ServiceActivity\manifests\$($package.Id)\capability.json"
...
Write-Host "Service Activity 三个能力包构建完成：$SamplesRoot"
