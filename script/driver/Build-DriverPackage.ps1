...
$projectRoot = Join-Path $repositoryRoot "drivers\EdrTestDriver"
$project = Join-Path $projectRoot "EdrTestDriver.vcxproj"
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $repositoryRoot "artifacts\driver-package\x64\$Configuration"
}
...
foreach ($required in @($setup, $msbuild, $signTool, $inf2Cat, $project)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "驱动构建依赖不存在：$required" }
}
...
if ($LASTEXITCODE -ne 0) { throw "EWDK 驱动构建失败，退出码 $LASTEXITCODE。" }

$builtDriver = Join-Path $projectRoot "bin\x64\$Configuration\EdrTestDriver.sys"
if (-not (Test-Path -LiteralPath $builtDriver -PathType Leaf)) {
    $builtDriver = Get-ChildItem $projectRoot -Filter EdrTestDriver.sys -File -Recurse |
...
if ([string]::IsNullOrWhiteSpace($builtDriver) -or -not (Test-Path -LiteralPath $builtDriver)) {
    throw "EWDK 构建完成但未找到 EdrTestDriver.sys。"
}

if (Test-Path -LiteralPath $OutputPath) { Remove-Item -LiteralPath $OutputPath -Recurse -Force }
[IO.Directory]::CreateDirectory($OutputPath) | Out-Null
$driverPath = Join-Path $OutputPath "EdrTestDriver.sys"
$infPath = Join-Path $OutputPath "EdrTestDriver.inf"
$catPath = Join-Path $OutputPath "EdrTestDriver.cat"
Copy-Item -LiteralPath $builtDriver -Destination $driverPath -Force
Copy-Item -LiteralPath (Join-Path $projectRoot "package\EdrTestDriver.inf") -Destination $infPath -Force
...
if (-not [string]::IsNullOrWhiteSpace($CertificateThumbprint)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "EdrTestDriver.sys 签名失败。" }
    $signed = $true
}

& $inf2Cat "/driver:$OutputPath" /os:10_X64,Server10_X64 /verbose
if ($LASTEXITCODE -ne 0) { throw "Inf2Cat 生成目录文件失败。" }
if (-not (Test-Path -LiteralPath $catPath)) { throw "Inf2Cat 未生成 EdrTestDriver.cat。" }

if ($signed) {
    & $signTool sign /v /sha1 $CertificateThumbprint /fd SHA256 $catPath
    if ($LASTEXITCODE -ne 0) { throw "EdrTestDriver.cat 签名失败。" }
}

$driverHash = (Get-FileHash -LiteralPath $driverPath -Algorithm SHA256).Hash.ToLowerInvariant()
$metadata = [ordered]@{
    schema_version = "1.0"
    package_name = "EdrTestDriver"
    architecture = "x64"
...
$metadata | ConvertTo-Json -Depth 10 | Set-Content (Join-Path $OutputPath "driver-package.json") -Encoding utf8NoBOM

if ($UpdatePrebuilt) {
    $prebuilt = Join-Path $projectRoot "prebuilt\x64"
    [IO.Directory]::CreateDirectory($prebuilt) | Out-Null
    foreach ($name in @("EdrTestDriver.sys", "EdrTestDriver.inf", "EdrTestDriver.cat", "driver-package.json")) {
        Copy-Item -LiteralPath (Join-Path $OutputPath $name) -Destination (Join-Path $prebuilt $name) -Force
    }
}

Write-Host "[已生成] 驱动包：$OutputPath"
