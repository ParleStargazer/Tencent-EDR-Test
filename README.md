...
准备好 .NET 8 SDK（或更高版本）、PowerShell 7、Node.js 22.13+ 和 pnpm 11.9+ 后，双击仓库根目录的 `启动平台.cmd`。入口提供“增量启动”和“全量重构启动”，5 秒内没有选择时默认进入增量模式。增量模式根据源码、清单、锁文件、工具链版本、驱动包和前端 API 地址的 SHA-256 指纹，只重新构建发生变化且输出仍完整的部分；缓存保存在 Git 忽略的 `.edr-test/build-cache/`。全量模式忽略指纹并重新生成 Runner、全部可用能力包和前端产物。两种模式都保留完整现场构建能力，随后启动本地服务并打开 `http://127.0.0.1:3000/`。
