...
public static class CompareService
{
    ...
    internal static string RequirementTitle(string field, string @operator)
    {
        var subject = field switch
        {
            ...
            "powershell.content" => "EDR 扫描到的 PowerShell 内容",
            "powershell.content_name" => "EDR 记录的 PowerShell 内容名称",
            "powershell.content_type" => "EDR 记录的 PowerShell 内容类型",
            "powershell.hook_module" => "EDR 使用的 PowerShell 扫描模块",
            "powershell.app_version" => "EDR 记录的 PowerShell 接口版本",
            "powershell.monitor_name" => "EDR 记录的 PowerShell 监控名称",
            "bits.job_id" => "EDR 记录的 BITS Job ID",
            "bits.display_name" => "EDR 记录的 BITS 任务名称",
            "bits.job_type" => "EDR 记录的 BITS 任务类型",
            "bits.state" => "EDR 记录的 BITS 任务状态",
            "bits.owner_sid" => "EDR 记录的 BITS 所有者 SID",
            "bits.remote_url" => "EDR 记录的 BITS 远端 URL",
            "bits.local_path" => "EDR 记录的 BITS 本地路径",
            "bits.bytes_total" => "EDR 记录的 BITS 总字节数",
            "bits.bytes_transferred" => "EDR 记录的 BITS 已传输字节数",
            "bits.notification_command" => "EDR 记录的 BITS 通知命令",
