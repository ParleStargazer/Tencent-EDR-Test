        ...
        ProgramObservation helper)
    {
        ...
        for (var index = 0; index < attempts.Count; index++)
        {
            ...
            var values = new Dictionary<string, JsonNode?>(StringComparer.Ordinal)
            {
                ...
                [$"{prefix}.started_at_utc"] = JsonValue.Create(Values.Utc(attempt.StartedAtUtc)),
                [$"{prefix}.completed_at_utc"] = JsonValue.Create(Values.Utc(attempt.CompletedAtUtc)),
                [$"{prefix}.duration_ms"] = JsonValue.Create(attempt.DurationMs),
