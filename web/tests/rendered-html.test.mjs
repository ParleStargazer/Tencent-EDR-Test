...
test("模板预览已移除且页面接入本地 Runner API", async () => {
  ...
  assert.match(livePage, /实际开始时间前 10 秒/);
  assert.match(livePage, /实际结束时间后 10 秒/);
  assert.match(livePage, /高级等待选项/);
  assert.match(livePage, /timing: cloudAutomationTiming/);
  ...
  assert.match(localApi, /result\.StartedAtUtc\.AddSeconds\(-10\)/);
  assert.match(localApi, /result\.EndedAtUtc\.AddSeconds\(10\)/);
  assert.match(cloudService, /step_timeout_ms/);
  assert.match(cloudService, /query_result_settle_ms/);
