...
const defaultActionNameInputs: Record<string, string> = {
  ...
  "win.service.create": "CreateService, CreateServiceW, RpcCreateService, ServiceInstall",
  "win.service.modify": "ChangeServiceConfig, ChangeServiceConfigW, ServiceConfigChange",
  "win.service.delete": "DeleteService, DeleteServiceW",
  ...
  "win.hash.md5": "FileWriteClose",
  "win.hash.sha": "FileWriteClose",
  "win.hash.imphash": "FileWriteClose",
...
export function LiveControlPlane({ view = "overview" }: { view?: ControlPlaneView }) {
  ...
  async function startRun() {
    ...
    try {
      ...
      setCloudPassword("");
      setActiveRun(run);
      setNotice(cloudAutomationEnabled ? `已提交 ${selectedIds.length} 项能力；本地结束后将自动获取并绑定云端日志。` : `已提交 ${selectedIds.length} 项能力。`);
      void refreshRuns();
    } catch (error) {
...
function Overview({ apiState, capabilities, baselines, recentRuns }: { apiState: ApiState; capabilities: ApiCapability[]; baselines: ApiBaseline[]; recentRuns: ApiRun[] }) {
  ...
  return <>
    ...
    <section className="journey-grid" aria-label="验证流程入口">
      <Link className="journey-card" href="/test"><span className="journey-number">步骤 1</span><h2>进行能力测试</h2><p>选择能力并运行，查看每一步进度、等待倒计时和控制程序日志。</p><strong>进入测试页面 <span aria-hidden="true">→</span></strong></Link>
...
function TestWorkspace(props: TestWorkspaceProps) {
  ...
  return <>
    <PageHeader index="02" eyebrow="进行测试" title="能力测试" description="能力按选择顺序执行；同一能力的多个子测试按设定间隔依次运行。" apiState={props.apiState} />
    ...
    <div className="workspace-grid test-setup-grid">
      ...
      <section className="panel run-panel sticky-panel">
        <div className="panel-heading"><div><p className="section-index">B / 运行设置</p><h2>启动本轮测试</h2></div><span className="line-badge">按序执行</span></div>
        <div className="form-stack">
          ...
          <div className="serial-note"><strong>执行顺序</strong><p>子测试按设定间隔依次运行；能力完成并写入 SQLite 后，开始下一项能力的等待倒计时。</p></div>
...
function RunProgressPanel({ run, onCancel, onDownload, onDownloadCloudDebugLog }: { run: ApiRun | null; onCancel: () => void; onDownload: (run: ApiRun) => void; onDownloadCloudDebugLog: (run: ApiRun) => void }) {
  return <section className="panel progress-panel"><div className="panel-heading"><div><p className="section-index">C / 测试进度</p><h2>{run ? run.phase : "等待测试开始"}</h2></div>{run && <span className="line-badge">{run.progress}%</span>}</div>
    {run ? <><div className="progress-track large"><span style={{ width: `${run.progress}%` }} /></div><div className="run-progress-meta"><span>已完成 {run.completed_capabilities}/{run.capability_ids.length}</span><span>{run.wait_remaining_seconds ? `下一项将在 ${run.wait_remaining_seconds} 秒后开始` : "按选择顺序执行"}</span><span>{run.database_name ?? "正在创建独立 SQLite"}</span></div>
...
function MethodComparison({ methods, selection, requirements, candidates, onOpenCandidate }: { methods: MethodResult[]; selection: MethodSelectionResult | null; requirements: BaselineRequirementResult[]; candidates: EdrCandidate[]; onOpenCandidate: (candidate: EdrCandidate) => void }) {
  ...
  return <section className="method-comparison-section"><header><div><span>EDR 子测试方法</span><h4>{singleMethod ? "测试方法通过情况" : "不同方法的通过情况"}</h4><p>{singleMethod ? "方法内分别展示本地绝对基准与对应的 EDR 要求。" : "每种方法内分别展示本地绝对基准与该方法对应的 EDR 要求；最佳方法默认展开。"}</p></div><em>{methods.length} 种方法</em></header>{selection && <div className="method-selection-notice"><strong>{singleMethod ? "已采用该方法形成结论" : "已采用最佳方法形成结论"}</strong><p>{selection.notice}</p></div>}<div className="method-result-list">{methods.map((method) => {
